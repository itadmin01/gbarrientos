...
class PaymentAcquirer(models.Model):
    ...
    def _get_feature_support(self):
        ...
        res['fees'].append('conekta')
        res['fees'].append('conekta_oxxo')
        return res
    
    @api.multi
    def conekta_compute_fees(self, amount, currency_id, country_id):
        return self.conekta_and_oxoo_compute_fees(amount, currency_id, country_id)
    
    @api.multi
    def conekta_oxxo_compute_fees(self, amount, currency_id, country_id):
        return self.conekta_and_oxoo_compute_fees(amount, currency_id, country_id)
            
    @api.multi
    def conekta_and_oxoo_compute_fees(self, amount, currency_id, country_id):
        """ Compute conekta fees.
...
class PaymentTransaction(models.Model):
    ...
    def _conekta_oxxo_form_validate(self, data):
        ...
        conekta.api_key = self.acquirer_id.conekta_secret_key_test if self.acquirer_id.environment=='test' else self.acquirer_id.conekta_secret_key 
    ...
    def create_params(self, acquirer):
        ...
        if self.fees:
            params['charges'][0].update({'fee':round(self.fees * 100)})
            
        details = params['customer_info'] = {}
        details['name'] = partner.name or self.partner_name or ''
        details['phone'] = partner.phone or ''
        ...
        if hasattr(self,'invoice_ids'):    
            ...
            for invoice in self.invoice_ids:
                ...
                for tax_line in invoice.tax_line_ids:
                    if (tax_line.tax_id.id, tax_line.name) not in tax_lines:
                        tax_lines[(tax_line.tax_id.id, tax_line.name)] = tax_line.amount 
                    else:
                        tax_lines[(tax_line.tax_id.id, tax_line.name)] = tax_lines[(tax_line.tax_id.id, tax_line.name)] + tax_line.amount
            #If price include taxes, than Amount need to pass untaxed amount
